<!--
    FamilyTreeEditor - Open tree dialog: searchable list + preview + open/delete
...
<script lang="ts">
    ...
    const { listings, activeId, onpick, ondelete, onclose, onnotice }: Props = $props();
...
>
    <div
        class="bg-canvas-elev border-line text-fg flex h-[70vh] w-full max-w-3xl flex-col rounded-lg border shadow-2xl"
    >
        <header class="border-line flex items-center justify-between border-b px-5 py-3">
            <h2 class="text-base font-semibold">open tree</h2>
            <button
                type="button"
