...
import type { LayoutNodeId, PlacedGraph, RoutedGraph } from "$lib/layout/ir";
...
export function route(placed: PlacedGraph, tree: Tree): RoutedGraph {
    return { placed, segments: buildSegments(placed, tree) };
}
...
function midX(p: Pos): number { return p.x + PERSON_W / 2; }
function midY(p: Pos): number { return p.y + CARD_H / 2; }
function topY(p: Pos): number { return p.y; }
function botY(p: Pos): number { return p.y + CARD_H; }
function leftX(p: Pos): number { return p.x; }
function rightX(p: Pos): number { return p.x + PERSON_W; }
...
class GutterLanes {
    ...
    alloc(gutterRank: number, minX: number, maxX: number): number {
        ...
        if (!list) { list = []; this.slots.set(gutterRank, list); }
        for (let lane = 0; lane < N_LANES + 16; lane++) {
            const blocked = list.some(
                (s) => s.lane === lane && s.maxX > minX && s.minX < maxX,
            );
            if (!blocked) { list.push({ minX, maxX, lane }); return lane; }
        }
        return 0;
    }
...
function buildSegments(placed: PlacedGraph, tree: Tree): readonly Segment[] {
    const out: Segment[] = [];
    ...
    for (const couple of tree.couples) {
        ...
        if (aPos.rank === bPos.rank) {
            ...
            if (isLongBond) {
                ...
                out.push({
                    id: `${bondIdBase}/stub-l`, kind: "stub", role,
                    x1: rightX(l), y1: bondY, x2: rightX(l) + STUB_LEN, y2: bondY,
                    persons: couplePersons,
                });
                out.push({
                    id: `${bondIdBase}/stub-r`, kind: "stub", role,
                    x1: leftX(r) - STUB_LEN, y1: bondY, x2: leftX(r), y2: bondY,
                    persons: couplePersons,
                });
            } else {
                out.push({
            ...
            out.push({ id: `${bondIdBase}/v1`, kind: "bond", role, x1: ux, y1: botY(upper), x2: ux, y2: midYVal, persons: couplePersons });
            ...
            if (Math.abs(lx - ux) > maxBondSpan) {
                ...
                out.push({ id: `${bondIdBase}/h`, kind: "bond", role, x1: ux, y1: midYVal, x2: lx, y2: midYVal, persons: couplePersons });
            }
        }
    }
    ...
    // -----------------------------------------------------------------------
    const byParent = new Map<PersonId, { parentPos: Pos; kids: Array<{ id: PersonId; pos: Pos }> }>();
    for (const person of Object.values(tree.people)) {
...
function annotateHops(segments: readonly Segment[]): Segment[] {
    ...
    for (const seg of segments) {
        if (!isV(seg)) { out.push(seg); continue; }
        ...
        if (hops.length === 0) { out.push(seg); continue; }
        hops.sort((a, b) => a - b);
        out.push({ ...seg, hops });
    }
    return out;
