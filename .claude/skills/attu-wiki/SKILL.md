...
This project's `.venv` already has `requests` and `mwparserfromhell` installed — prefer the Python workflow over raw curl + regex when you need anything beyond a one-shot lookup.

## First reach: `scripts/wiki.py`

For the common operations, **do not write Python from scratch**. `scripts/wiki.py` wraps them as subcommands. Run with the project venv:

```bash
.venv/bin/python scripts/wiki.py <subcommand> [args]
```
...
```bash
.venv/bin/python scripts/wiki.py extract "Kingdom of Tietero" --intro
.venv/bin/python scripts/wiki.py infobox "Kingdom of Tietero"
.venv/bin/python scripts/wiki.py members "Tietero pages" | head
.venv/bin/python scripts/wiki.py batch --from-category "Tietero pages" > /tmp/tietero.json
printf 'Zamenhof\nAvanguardo\n' | .venv/bin/python scripts/wiki.py exists
```
...
Always invoke with the project venv: `.venv/bin/python script.py` (or `.venv/bin/python -` for stdin).

A small reusable client — copy into a script or paste into `.venv/bin/python -`:
