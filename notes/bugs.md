...
- :o: `medium priority` `low effort` family-view edges become too thin to see when zoomed out - strokes already carry `vector-effect="non-scaling-stroke"` (e.g. `FamilyViewCanvas.svelte:895/908/927/942/949/957/965/972/977/991`), so the nominal width holds in screen-px but reads as "too thin" against shrunken cards at low zoom. fix: either bump the base stroke width, or add a mild zoom-aware bump (e.g. `max(BASE_PX, floor / scale)`) so edges grow proportionally as the viewport zooms out and stay legible. mirror to TreeCanvas / HyperbolicCanvas if the same complaint applies there.
