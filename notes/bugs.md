...
- :o: `high priority` `medium effort` family-view couple/marriage bonds route on top of cards rather than around them - horizontal bond segments are painted at a y that passes through one or more cards above/below the couple pair, so the line visibly crosses person tiles instead of running through the inter-rank gutter. analogous in shape to the layered-engine "multi-spouse bond passes through intervening ghost card" bug below but on family-view geometry. start in `FamilyViewCanvas.svelte` edge emission - confirm whether bonds get a dedicated y-lane in the gutter and whether card AABBs are even considered as obstacles.
...
- :o: `medium priority` `low effort` coefficient-of-inbreeding (COI) value shown in the inspector / connections tab looks like it may have a rounding error - user-reported, no concrete repro yet. Wright's-formula impl lives in `lib/domain/consanguinity.ts`; needs a numeric audit (accumulation order, `2^-n` precision at deep pedigrees, display rounding in the consumer). when fixing, add unit tests in `tests/unit/domain/consanguinity.test.ts` for the failing cases plus a few hand-computed pedigrees at the precision boundary so regressions get caught.
...
- :red_circle: `24 May 2026` family-view "N people" stats pill mounts in the wrong corner - extend `statsPillVisible` gate in `App.svelte` to include `selectedEngine === "family-view"` so the pill mounts in the shared bottom-left `data-canvas-chrome` bar alongside the save-status + debug pills; family-view's components=1 / isolated=0 emission renders as a compact "N people" with no cluster suffix.
