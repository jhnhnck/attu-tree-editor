...
- 🔴 `23 May 2026` `visual-path-highlight-multi-union.png` snapshot dimension mismatch (expected 920×806, received 920×1241) - snapshot rebased to 920×1241 at commit `230711e` (family-view-w2 phase 1); all 87 test files pass on trunk. not a regression.
- 🔴 `23 May 2026` `card-height.test.ts` referenced `CARD_H_COMPACT` after `cardHeight()` simplified to portrait-only - stale assertions rewritten to use `CARD_H`; `CARD_H_COMPACT` constant removed. verified: `pnpm test:unit` passes 967/967 on trunk.
