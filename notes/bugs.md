...
- ⭕ `high priority` `medium effort` 12% of drops have negative height - `(y2 - y1)` for parent-drop / child-drop should always be ≥ 0 (parent above child). On the DEMO fixture 353/2813 drops are negative (heights -0.1 to -2.3u), and 23 are >2.0u (parent two ranks above child). Top offending bonds: single parents Kobartan Banokan (7), Tratoi Bokok (4), Othataim Skrake (4), Fomakta Arkara (4), Obantar Skrake (4), Drachalur Dar (4), Bok Deram (4); plus the couple Lazaint Oken & Kozakar Oken (4). Likely root: `passes/layer.ts` choosing a parent rank below the child rank when the child has multiple parents on different layers, or `choosePrimary` not handling single-parent rank assignment for cross-rank cases. The 14 tall (>2.0) drops are exclusively `single:` parents - one-parent vertical spans skip ghost insertion. Phase 0 added the invariant assertion in `route.ts` (logs a `console.warn` per offending segment); the layer.ts root-cause fix remains open 🎯 *planned in [relationship-vocabulary.md](../plans/relationship-vocabulary.md)*
- ⭕ `high priority` `low effort` spurious ghost on the top rank after "add parent" when the added-to person's spouse has no parents - `passes/layer.ts` `computeRanks` (lines 294-354) is a longest-path BFS over the parent-child DAG only; spouse edges are ignored. Repro on a 6-person tree (Korak + Wife + 3 children): invoke "add parent" on Korak → New Person lands at rank 0, Korak moves to rank 1, but Wife Nokar (no parents) stays at rank 0. The cross-rank ghost logic at lines 113-133 sees `rankL=1 ≠ rankR=0` and inserts a ghost of Wife on the top row next to New Person, even though there is no in-law relationship. `App.svelte:384-394` `addParent` correctly calls `linkParent` only and does NOT create a phantom couple, so the bug is purely in rank assignment. Fix: after the parent-DAG BFS, run a couple-equalisation pass that raises the lower-ranked partner to match the higher-ranked one (never lower the parented partner, that would break the parent-drop edge). Same root cause likely contributes to the negative-drop bug above 🎯 *planned in [relationship-vocabulary.md](../plans/relationship-vocabulary.md)*
- ⭕ `low priority` `low effort` cross-rank couple orientation remains order-driven after the Phase 1 father-left tie-break in `passes/order.ts`. The tie-break is 100% effective on same-rank both-known couples (37/37 on the Akarians DEMO fixture) but cannot reach cross-rank pairs because their real-person X positions are decided by independent per-rank ordering — `repairCoupleAdjacency` only operates within a rank. To extend the convention to cross-rank couples too, `passes/layer.ts` would need a couple-equalisation step (the same one the relationship-vocabulary plan's Phase 2 schedules for the negative-drop bug). Until then, on Akarians the overall both-known male-left ratio sits at ~57% (100% on same-rank + ~50% on cross-rank). Original bug:18 (same-rank inversion) is fixed; this is the residual cross-rank scope. 🎯 *planned in [relationship-vocabulary.md](../plans/relationship-vocabulary.md) Phase 2*
...
_no fixed items yet; when one is closed, move it here with a date stamp and strip the priority/effort tags._
