...
| `src/lib/io/familyecho-html/`   | import-only parser for family echo `.html` exports (`parse.ts`)                                                                                                                                                                                                                                                                                                                                                                      |
...
| `src/lib/io/import/`            | `composeImports.ts` - wires format detection, parsing, and merge into the import wizard's pipeline                                                                                                                                                                                                                                                                                                                                  |
| `src/lib/io/detect.ts`          | filename + magic-byte format sniffer                                                                                                                                                                                                                                                                                                                                                                                                 |
| `src/lib/io/warnings.ts`        | per-target `fieldsDroppedFor()` helper                                                                                                                                                                                                                                                                                                                                                                                               |
| `src/lib/io/importFile.ts`, `persistImportPayload.ts` | top-level glue: read a dropped/picked file, detect + parse it, then persist the resulting payload                                                                                                                                                                                                                                                                                                                  |
...
| `src/lib/state/`                | runes-based stores: `tree.svelte.ts` (snapshot undo/redo + dirty flag), `selection.svelte.ts`, `viewport.svelte.ts`, `portraitUrls.svelte.ts` (blob -> object-URL cache), `sync.svelte.ts` (server revision tracking), `preferences.svelte.ts`, `engine.ts` (active layout engine), `preferredUnionMigration.ts`; `autosave.ts` debounces tree changes into Dexie writes. `toasts.svelte.ts` / `progress.svelte.ts` / `auth.svelte.ts` moved to `@attu/ui` - import `createToastsStore` / `createProgressStore` / `authStore` from there, not from this directory |
...
| `src/lib/components/tree/`      | `TreeCanvas.svelte`, `FamilyViewCanvas.svelte`, `HyperbolicCanvas.svelte` (per-engine renderers), `PersonNode.svelte` (foreignObject card), `EdgeLayer.svelte` + `edgePath.ts` (svg connectors), `canvasController.ts` (pan/zoom/focus), `DebugOverlay.svelte` / `FamilyViewDebugOverlay.svelte` (Ctrl+Shift+D), `InstancePopover.svelte`                                                                                            |
| `src/lib/components/editor/`    | `PortraitField.svelte` (upload + thumb) only. `PersonEditor.svelte` was retired (editing now happens through the tabbed inspector below); `CropperDialog.svelte` / `CropperCanvas.svelte` / `cropperMath.ts` / `loadSourceBitmap.ts` / `encodePortrait.ts` (canvas-based portrait cropper; cropperjs removed) all moved to `@attu/ui`'s `components/editor/`                                                                       |
| `src/lib/components/import/`    | `ImportWizard.svelte` - the import wizard (FamilyScript/.ged/.gdz, dual-import merge UI)                                                                                                                                                                                                                                                                                                                                              |
...
| `src/lib/components/shell/`     | `OpenDialog.svelte` (app-specific) + `saveStatusGlyph.ts` only. `MenuBar.svelte` / `Menu.svelte` / `Shell.svelte` (the action registry + menu chrome), `AuthBar.svelte`, `AdminPanel.svelte`, `ProgressStrip.svelte`, `ShareDialog`, `SettingsDialog`, `LinkCodeDialog`, `AboutDialog` all moved to `@attu/ui`'s `components/shell/` - both SPAs import the same `Shell` pattern from there now. save-status + stats pills live in the `@attu/ui` dock                                                                                                                                                      |
| `src/lib/components/palette/`   | `commands.ts` only (Ctrl+P quick-jump + Ctrl+Shift+P command list). `CommandPalette.svelte` itself moved to `@attu/ui`'s `components/palette/`                                                                                                                                                                                                                                                                                       |
...
note: `components/canvas/`, `components/help/`, `components/form/`, and `components/ui/` no longer exist under `apps/tree-editor/src/lib/` - `ZoomWidget.svelte` / `BackButton.svelte`, `ShortcutsOverlay.svelte`, `DateInput.svelte` / `Field.svelte`, and `Button.svelte` all now live under the matching `packages/attu-ui/src/lib/components/{canvas,help,form,ui}/` and are shared by both SPAs.
...
- **component**: `apps/tree-editor/tests/component/**/*.test.ts` - vitest + jsdom + `@testing-library/svelte`; 37 spec files. includes the structural-invariant suites (chrome-geometry, auto-fit-suppression, parity-matrix, selection-state-machine, keyboard-reachability) that replaced the deleted visual-snapshot floor - see [`notes/features/test-invariants.md`](features/test-invariants.md). `tests/setup.ts` registers jest-dom matchers, per-test cleanup, and no-op global shims for `ResizeObserver` / `IntersectionObserver` / `window.matchMedia` (jsdom 29.1.1 omits all three). `vitest.config.ts` sets `resolve.conditions: ['browser']` so the svelte plugin returns the client build, not the SSR one
- **e2e**: removed (`chore(test): remove e2e suite pending app maturity`, 18 June 2026) - playwright and the `tests/e2e/` tree are gone; no `playwright.config.ts`, no e2e dependency. coverage that used to live in e2e specs now needs to be carried by component tests or revisited if e2e is reintroduced
- **harnesses**: `apps/tree-editor/tests/component/_harness/` collects reusable mount sandwiches and helpers - `loadGedcomFixture`, `mountWithHostRect`, `stackingContextHitTest`, `StatsPillHarness`, `ImportEditHarness`, `GenderFieldHarness`, `AgabFieldHarness`, `ParentPickerHarness`, `PortraitFieldHarness`. extract a new harness when a second spec needs the same scaffold. `MenuHarness` / `TabOrderHarness` / `ToggleMenuHarness` moved to `packages/attu-ui/tests/component/_harness/` (commit `1dd0e7d`, 19 June 2026) along with the cluster-3 toggle-indicator suite and the `DateInput` keyboard-reachability spec, since those components now live in `@attu/ui`
- `pnpm test:unit` runs vitest (unit + component, the only test layer for the web side right now)
...
```bash
pnpm dev             # caddy :8000 fronting tree-editor :5173, wiki-editor :5174, fastapi :8001 - see scripts/dev.zsh
pnpm tree-server:dev # uvicorn --reload on :8000 (standalone, no caddy/wiki-editor)
...
18. **debug mode vs debug menu open**: two pieces of state drive the debug surface, not one. `debugMode` is the master switch - persisted in `localStorage["fte.debug.mode"]` (plain boolean). it gates the debug pill's registration and both `debugOptions` / `familyViewDebugOptions` derivations (so layered + family-view overlays only render when debug mode is on). the debug menu's open-state is separate - `dockStore.isOpen("debug-menu")` (non-persisted; the `debug-menu` id never round-trips to `fte.dock.openedPanels`). `Ctrl+Shift+D` toggles it. closing the menu via `×` leaves overlays running. flipping `debugMode` off closes the menu as a side effect; the "disable debug mode" button in the menu body does the same. closing the menu does NOT clear overlays.
19. **dock placement keys**: the dock persists two localStorage keys (no schema version; client-only ui state). `localStorage["fte.dock.corner"]` holds the active corner (`"tl" | "tr" | "bl" | "br"`); `localStorage["fte.dock.openedPanels"]` is a JSON array of open panel ids - non-persistent ids (debug-menu + every family-view-debug-* panel) are filtered out on write. drag-to-reorder pill `order` is in-memory only - not persisted.
20. **web workers can't process svelte files**: a worker build context (e.g. `layout.worker.ts`) cannot import `.svelte` files even through a workspace package like `@attu/ui` - the worker bundler has no svelte-aware transform. pure-TS utilities meant for worker-visible code paths need their own export subpath, separate from the package's component exports. applies to any new `@attu/ui` utility a worker needs.
21. **`bind:this` targets need `$state()` if passed down as a prop**: in svelte 5 runes mode, a plain `let component: T | undefined;` bound via `bind:this={component}` gets assigned correctly (the value updates), but that assignment doesn't propagate through svelte's reactivity graph - any sibling component that receives `component` as a prop (e.g. `<Toolbar {editor} />` where `editor` is bound from a sibling `<Editor bind:this={editor} />`) will see `undefined` forever, since nothing tells the runtime to re-evaluate the prop after the bind:this fires post-mount. fix: `let component: T | undefined = $state();`. a plain `let` bound via `bind:this` is fine as long as it's only ever read imperatively from an event handler (not passed as a prop or read in a template/`$derived`) - `apps/tree-editor/src/App.svelte`'s `shellRef` is a correct example of that narrower case. `apps/wiki-editor/src/App.svelte`'s `editor` binding needed the `$state()` fix when it started being passed to `Toolbar`/`SelectionBar` (wiki-editor-format-commands plan, phase 0).
...
**`notes/profiles/`** - performance profiling snapshots, kept as reference fixtures, not living docs

- [`notes/profiles/debug-menu-open-lag-baseline.md`](profiles/debug-menu-open-lag-baseline.md), [`debug-menu-open-lag-after.md`](profiles/debug-menu-open-lag-after.md) - before/after profile of the debug-menu open-lag fix

**`notes/reports/`** - point-in-time audit/completion snapshots, not living docs

- [`notes/reports/audit-cluster-01.md`](reports/audit-cluster-01.md) through `audit-cluster-09.md` - dated multi-agent audit snapshots
- [`notes/reports/completed-items-report.md`](reports/completed-items-report.md) - dated completed-items snapshot
...
│   │   │   └── lib/
│   │   │       ├── components/
│   │   │       │   ├── editor/               # PortraitField only (cropper moved to @attu/ui)
│   │   │       │   ├── import/               # import wizard
│   │   │       │   ├── inspector/            # tabbed sidebar
│   │   │       │   ├── palette/               # commands.ts only (CommandPalette.svelte moved to @attu/ui)
│   │   │       │   ├── shell/                # OpenDialog + saveStatusGlyph only (menu/chrome moved to @attu/ui)
...
│   │   │   ├── app.css
│   │   │   ├── main.ts
│   │   │   └── lib/                          # Editor (CodeMirror 6), Toolbar, SelectionBar, SettingsModal, ShortcutsOverlay, lang-wikitext/, state/preferences.svelte.ts
│   │   ├── index.html
│   │   ├── package.json
│   │   ├── svelte.config.js
│   │   ├── tsconfig.json
