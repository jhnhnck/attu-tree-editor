...
- ⭕ `medium priority` `medium effort` zoom-aware label sizing - shrink card padding and grow text size as zoom decreases so the next-level-up card stays readable as long as possible (PersonNode + the `levelFromScale` thresholds in TreeCanvas) 🎯 *partially obsoleted by family-view plan — default-view users see readable cards at fit zoom by design; still relevant to the layered / hyperbolic alternative views (see [notes/plans/family-view.md](plans/family-view.md))*
- ⭕ `medium priority` `high effort` _conditionally claimed by [relationship-vocabulary plan](plans/relationship-vocabulary.md) Phase 4 — promoted into scope iff Phase 1's 30-overlay readability probe on Akarians shows overlay routing is unreadable. Otherwise stays open here as a follow-up._ orthogonal edge routing with obstacle avoidance - even after spouse-duplication, some long bonds + sibling-bus segments still pass through other cards. The multi-spouse and negative-drop defects in [bugs.md](bugs.md) are concrete instances; the same issue shows up for sibling-bus segments that span past intervening cards above/below the row. Implement A* over a sparse routing graph (corners of card AABBs + row-gutter alignment lines) so edges bend around any card they would otherwise visually cross. Polish layer; the targeted bug fixes are higher-ROI prerequisites 🎯 *partially obsoleted by family-view plan — bounded default view has too few cards to crowd; still load-bearing for layered / hyperbolic and (per relationship-vocabulary work) for overlay-edge routing once overlays exist (see [notes/plans/family-view.md](plans/family-view.md), [notes/features/relationship-vocabulary.md](features/relationship-vocabulary.md))*
- ⭕ `medium priority` `high effort` hide unrelated branches based on the selected person - needs a "related-to" rule (default: ancestors + descendants + spouses); expose as a View menu toggle so users can flip between full tree and focused view 🎯 *claimed by family-view plan — the bounded default view IS the focused-view mode, with inline expand/collapse for refinement (see [notes/plans/family-view.md](plans/family-view.md) Phase 0 + Phase 1)*
- ⭕ `medium priority` `medium effort` selectable lineage trace - clicking an edge (or a person + an "trace" action) highlights a chain through the graph in a unique color so the user can see where a relationship goes; pairs naturally with the "hide unrelated branches" toggle 🎯 *claimed by family-view plan — path highlight on selection is Phase 3 (see [notes/plans/family-view.md](plans/family-view.md))*
...
- ⭕ `medium priority` `low effort` "add relative" should pre-fill the new person's surname from the anchor person (parent / partner / child / sibling all share a surname by default); user has to retype it every time 🎯 *claimed by family-view plan — the new `+` affordances in Phase 4 should bake this in (see [notes/plans/family-view.md](plans/family-view.md) Phase 4)*
- ⭕ `medium priority` `low effort` PersonNode portrait area is too short - the portrait crop reads as a thin band rather than a face; either grow the portrait slot vertically or rebalance card padding so the face has proportional height to the name/dates block 🎯 *claimed by family-view plan — Phase 5 makes portrait-on-top the default card layout with proper proportions (see [notes/plans/family-view.md](plans/family-view.md) Phase 5)*
...
- ⭕ `low priority` `low effort` _claimed by [relationship-vocabulary plan](plans/relationship-vocabulary.md) Phase 8 ship polish._ `.claude/skills/schema-evolution/` skill - capture the migration pattern for the queued schema bumps (parents 1.0.0→2.0.0, unions 2.0.0→3.0.0, relationships 3.0.0→3.1.0, identity 3.1.0→3.2.0, groups 3.2.0→3.3.0, sibship 3.3.0→3.4.0): semver registry shape in `domain/schema.ts`, GEDZIP `manifest.json` stamping, semver-aware refusal-of-major-newer-than-build / accept-minor-newer-with-finding on read, the v(N-1) round-trip test convention. The relationship-vocabulary plan delivers six migrations as the canonical examples to point at.
...
🤝 *all items in this section coordinated with the relationship-vocabulary design study ([notes/features/relationship-vocabulary.md](features/relationship-vocabulary.md)) and the family-view plan's forward-compatibility contract ([notes/plans/family-view.md](plans/family-view.md)). Family View does not claim or block these — it commits to N-partner-ready IR shape, neutral editing vocabulary, and composable card decoration so any of these bumps can land independently without engine rework.*

🟡 *all four schema-evolution items below are now claimed by the [relationship-vocabulary plan](plans/relationship-vocabulary.md). They remain `⭕ open` here until the absorbing phase ships, at which point they move to the completed section with the date of merge.*

- ⭕ `future idea` `high effort` _claimed by relationship-vocabulary Phase 2 (1.0.0 → 2.0.0)._ replace `motherId` / `fatherId` with `parentIds: PersonId[]` (each entry carries optional `role: 'mother' | 'father' | 'parent' | 'progenitor'` and `pedi: 'birth' | 'adopted' | 'foster'`); supports asexual / multi-parent / non-binary single parents; ships with a v1 -> v2 migration in `domain/schema.ts`
- ⭕ `future idea` `medium effort` _claimed by relationship-vocabulary Phase 4 (3.0.0 → 3.1.0, additive)._ add generic `relationships: { kind: 'transformed-from' | 'alias-of' | 'sworn-bond' | 'master-apprentice' | ...; targetId; notes? }[]` for transmutation, alias, adoption-not-yet-mapped, and other fictional bonds; ships with a v2 -> v3 migration
- ⭕ `future idea` `low effort` _claimed by relationship-vocabulary Phase 6b (3.3.0 → 3.4.0, additive)._ add `birthOrder?: number` on `Person` for twin / triplet / cohort ordering inside a sibship (currently lost — sibship is derived from shared parents only); ships with a v3 -> v4 migration
- ⭕ `future idea` `low effort` _claimed by relationship-vocabulary Phase 3 (as `UnionRecord.name?`, part of 2.0.0 → 3.0.0)._ add optional `name?: string` to `CoupleRecord` so families can be referenced by a chosen surname / household name (currently no way to rename families); ships with a v? -> v? migration and an Inspector Connections-tab UI to set it
