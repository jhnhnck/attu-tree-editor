...
- :o: `medium priority` `low effort` SaveStatusPill popover anchors `absolute right-0 top-full mt-1` which renders below the viewport now that the pill mounts in the bottom-left bar - flip to `bottom-full mb-1 left-0` (or `right-0`) so the popover opens upward and stays on-screen. follow-up from the 24 May 2026 physical-move. re-flagged 24 May 2026.
