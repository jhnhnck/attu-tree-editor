...
- :o: `medium priority` `medium effort` initial tree load flashes the default sample tree for ~1s before the real tree paints - block rendering until the loaded tree resolves, or paint a generic spinner over an empty canvas
...
- :red_circle: `29 Jun 2026` user-settings dialog - `SettingsDialog` landed in `@attu/ui` and is wired into `App.svelte`; covers theme override, inspector side, and ergonomic toggles with localStorage persistence.
- :red_circle: `29 Jun 2026` unified loading-bar / progress indicator - `ProgressStrip` landed in `@attu/ui` and is wired into `App.svelte`; replaces scattered toast patterns for long operations.
- :red_circle: `29 Jun 2026` debug mode hidden by default on fresh load - `debugMode` persisted as `fte.debug.mode` now defaults to `false` (only `raw === "true"` enables it); old `debugPillHidden` var removed in the debug-system redesign.
- :red_circle: `29 Jun 2026` menu bar titles render lowercase - top-level menu labels (file / edit / view / insert / tree / help) and their entries changed to lowercase strings in `App.svelte` (`b3e0a72`).
