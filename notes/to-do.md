...
- :red_circle: `24 May 2026` SaveStatusPill popover now opens upward from the bottom-left chrome bar - flipped the anchor classes from `absolute right-0 top-full mt-1` to `absolute bottom-full left-0 mb-1` so the dropdown stays on-screen after the 24 May physical-move into the bottom bar.
- :red_circle: `24 May 2026` Insert menu and command palette now expose `person.addSibling` alongside the existing parent / partner / child / unattached commands - reuses the `addSibling()` function from the canvas context menu and mirrors its disabled affordance via a `personAddSiblingEnabled` flag (selection required + anchor must carry at least one parent ref).
